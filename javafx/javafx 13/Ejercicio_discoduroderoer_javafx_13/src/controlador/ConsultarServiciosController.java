...
public class ConsultarServiciosController implements Initializable {
    ...
    @FXML
    private void filtrarFechaInicial(ActionEvent event) {
        filtrarServicios();
    }
    ...
    @FXML
    private void filtrarFechaFinal(ActionEvent event) {
        filtrarServicios();
    }
    ...
    private void filtrarServicios() {
        ...
        LocalDate fechaAlquiler = this.dtpFechaInicial.getValue();
        LocalDate fechaFinal = this.dtpFechaFinal.getValue();
        Cliente c = this.cmbClientes.getValue();
        String NIF = null;

        // Si el cliente esta seleccionado, cojo su NIF
        if (c != null) {
            NIF = c.getNIF();
        }
        ...
        // Filtramos
        ObservableList<Servicio> items = s.getServicios(fechaAlquiler, fechaFinal, NIF);
        this.tblServicios.setItems(items);
